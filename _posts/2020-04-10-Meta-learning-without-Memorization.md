...
Let's come back to the student example at the beginning. The daily study at school can be considered as a task in meta-training, and we assume the student learn to solve one type of math problem each day. After a period, assume all types of required problems have been seen several times in class $($like what we once did for an important exam 💆$)$. If a student can remember all types of problems, he/she does not need to attend the class and can answer the homework questions of these problem types correctly. Though such seemingly innocuous memorization approach can solve new problems of the known types, however, when the student faces an unseen type of problem, he/she cannot solve it without learning from new resources.
